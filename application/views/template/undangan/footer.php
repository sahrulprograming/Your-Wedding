...
<!-- Script Pribadi -->
<script src="<?= base_url('assets'); ?>/undangan/js/app.js"></script>
<script>
    // Play & pause Music
    ...
    iconMusic.onclick = function() {
        if (!music.paused) {
            music.pause();
            iconMusic.src = "<?= base_url('assets'); ?>/undangan/audio/mute.svg"
        } else {
            music.play();
            iconMusic.src = "<?= base_url('assets'); ?>/undangan/audio/volume.svg"
        }
    }
</script>
<script>
    var waktu = new Date()
    let sekarang = waktu.getFullYear() + '-' + (waktu.getMonth() + 1) + '-' + waktu.getDate();
    ...
    var x = setInterval(function() {
        ...
        if (distance <= 0) {
            clearInterval(x);
            document.getElementById("hitung-mundur").innerHTML = "SUDAH BERLANGSUNG";
        }
    }, 1000);
</script>
</body>
